<?php
include "koneksi.php";

if (!isset($_SESSION['username'])) {
 header("Location: login_view.php");
}
if (!isset($_SESSION['level'])) {
  header("Location: login_view.php");
  # code...
}
?>
<head>
  <meta charset="utf-8" />
  <meta http-equiv="X-UA-Compatible" content="IE=edge" />
  <title>KaryaMuda-Komunitas-Berita</title>

<meta name="viewport" content="width=device-width, initial-scale=10">
  <link rel="stylesheet" href="css/styleku.css">
  <link href='logo.png' style="width: 32px;height: 32px;" rel='shortcut icon'>

</head>
<body>

<div class="atas">
      <div id="logopojok">
      <a href="logout.php"><img src="img/komunitas.png" title="logout"></a>
    </div>
    <div id="cari">
      <input type="text" id="cariin" name="cari"  value="" >
    </div>
    <div id="opsi">
      <button class="button button1">Kota</button>
      <button class="button button2">Nasional</button>
</div>
<div class="all">
  <div id="kiri">
    <div id="logo">
      <img src="img/komunitas.png">
    </div>
    <div id="profil">
      <div id="hprofil">
        Profil
      </div>
      <div id="iprofil">
        <p><b>KaryaMuda</b></p>
        <p>
          <?php
          echo $_SESSION["username"]
         ?>
         </p>
      </div>
    </div>
    <div id="halaman">
      <div id="hlaman">
        Halaman
      </div>
      <div id="ilaman">
        <div class="btn-group">
           <button class="button"><img src="img/notif.png">Notifikasi</button>
         <button class="button"><img src="img/berita.png">Berita</button>
         <a href="komunitas_ide dan karya.html"><button class="button"><img src="img/idekarya.png">Ide dan Karya</button></a>
         <a href="komunitas_solusi"><button class="button"><img src="img/solusi.png">Solusi</button></a>
         <a href="komunitas_relawan"><button class="button"><img src="img/relawan.png">Relawan</button></a>
      </div>
      </div>
    </div>
  </div>
  <div id="tengahim">
    <div id="htengahim">
    <div class="btn-group2">
    <button class="button buttona">Pendidikan</button>
    <button class="button buttona">Sosial</button>
    <button class="button buttona">Pariwisata</button>
    </div>
    </div>
    <div id="bgkonten">
    <div id="hkonten">
      <a href="komunitas_inputberita.php"></a><button id="addberita" > Tambah Berita </button></a>
    </div>
    <div id="kontenb">
      <section id="section-1"> <!-- ini nanti buat nulis berita nya section nya dilooping sesuai banyaknya berita  --> 
      <div class="konten">
        <div id="bgAtas">
          <div id="logokecil"><img src="img/komunitas.png"></div>
          <div id="komunitass"><p><b>KaryaMuda</b></p></div>
          <div id="nmkomunitass"><p>asal komunitas</p></div> <!-- ganti nama sesuai asal komunitasnya -->
          <div id="statusberita"><p>Statusnya?</p></div>
        </div>
        <div id="bgBawah"> 
          <div id="foto"><img src=""></div> <!-- isi sesuai yang sudah diupload pada form  -->
          <div id="isiberita"><p>
              <h2>ini judulnya</h2> 
              <p>isi paragraf ini isi paragraf ini isi paragraf ini isi paragraf ini isi paragraf ini isi paragraf ini isi paragraf ini
                isi paragraf ini visi paragraf ini v isi paragraf ini isi paragraf ini isi paragraf ini isi paragraf ini isi paragraf ini isi paragraf ini isi paragraf ini</p> <!-- isi sesuai konten  yang ada pada form -->
                <b><a href="">Lanjutkan..</a></b>
          </div>
        </div>
      </div>
      </section>
      <section id="section-2"> <!-- ini nanti buat nulis berita nya   --> 
      <div class="konten">
        <div id="bgAtas">
          <div id="logokecil"><img src="img/komunitas.png"></div>
          <div id="komunitass"><p><b>KaryaMuda</b></p></div>
          <div id="nmkomunitass"><p>asal komunitas</p></div> <!-- ganti nama sesuai asal komunitasnya -->
          <div id="statusberita"><p>Statusnya?</p></div>
        </div>
        <div id="bgBawah"> 
          <div id="foto"><img src=""></div> <!-- isi sesuai yang sudah diupload pada form  -->
          <div id="isiberita"><p>
              <h2>ini judulnya</h2> 
              <p>isi paragraf ini isi paragraf ini isi paragraf ini isi paragraf ini isi paragraf ini isi paragraf ini isi paragraf ini
                isi paragraf ini visi paragraf ini v isi paragraf ini isi paragraf ini isi paragraf ini isi paragraf ini isi paragraf ini isi paragraf ini isi paragraf ini</p> <!-- isi sesuai konten  yang ada pada form -->
                <b><a href="">Lanjutkan..</a></b>
          </div>
        </div>
      </div>
      </section>
      <section id="section-3"> <!-- ini nanti buat nulis berita nya   --> 
      <div class="konten">
        <div id="bgAtas">
          <div id="logokecil"><img src="img/komunitas.png"></div>
          <div id="komunitass"><p><b>KaryaMuda</b></p></div>
          <div id="nmkomunitass"><p>asal komunitas</p></div> <!-- ganti nama sesuai asal komunitasnya -->
          <div id="statusberita"><p>Statusnya?</p></div>
        </div>
        <div id="bgBawah"> 
          <div id="foto"><img src=""></div> <!-- isi sesuai yang sudah diupload pada form  -->
          <div id="isiberita"><p>
              <h2>ini judulnya</h2> 
              <p>isi paragraf ini isi paragraf ini isi paragraf ini isi paragraf ini isi paragraf ini isi paragraf ini isi paragraf ini
                isi paragraf ini visi paragraf ini v isi paragraf ini isi paragraf ini isi paragraf ini isi paragraf ini isi paragraf ini isi paragraf ini isi paragraf ini</p> <!-- isi sesuai konten  yang ada pada form -->
                <b><a href="">Lanjutkan..</a></b>
          </div>
        </div>
      </div>
      </section>
      </div>

      
    </div>
  </div>
  <div id="kanan">
     <center><H2>Top User</H2></center>
  <div id="rating1"> <!-- dilooping 5x kalau bisa kalau g bisa ya gini aja gpp :D  -->
    <center><img src="img/mahasiswa.png"></center>
    <div id="rat">Rating</div> <!-- top rating  -->
    <div id="namamhs"><b>Nama Mahasiswa</b></div>
    <div id="totalsol">Total solusi</div>
    <div id="totalrel">Total relawan</div>
  </div>
  <!-- sampai ini -->
  <div id="rating1"> <!-- dilooping 5x kalau bisa kalau g bisa ya gini aja gpp :D  -->
    <center><img src="img/mahasiswa.png"></center>
    <div id="rat">Rating</div> <!-- top rating  -->
    <div id="namamhs"><b>Nama Mahasiswa</b></div>
    <div id="totalsol">Total solusi</div>
    <div id="totalrel">Total relawan</div>
  </div>
  <div id="rating1"> <!-- dilooping 5x kalau bisa kalau g bisa ya gini aja gpp :D  -->
    <center><img src="img/mahasiswa.png"></center>
    <div id="rat">Rating</div> <!-- top rating  -->
    <div id="namamhs"><b>Nama Mahasiswa</b></div>
    <div id="totalsol">Total solusi</div>
    <div id="totalrel">Total relawan</div>
  </div>
  <div id="rating1"> <!-- dilooping 5x kalau bisa kalau g bisa ya gini aja gpp :D  -->
    <center><img src="img/mahasiswa.png"></center>
    <div id="rat">Rating</div> <!-- top rating  -->
    <div id="namamhs"><b>Nama Mahasiswa</b></div>
    <div id="totalsol">Total solusi</div>
    <div id="totalrel">Total relawan</div>
  </div>
  <div id="rating1"> <!-- dilooping 5x kalau bisa kalau g bisa ya gini aja gpp :D  -->
    <center><img src="img/mahasiswa.png"></center>
    <div id="rat">Rating</div> <!-- top rating  -->
    <div id="namamhs"><b>Nama Mahasiswa</b></div>
    <div id="totalsol">Total solusi</div>
    <div id="totalrel">Total relawan</div>
  </div>
  <div id="rating1"> <!-- dilooping 5x kalau bisa kalau g bisa ya gini aja gpp :D  -->
    <center><img src="img/mahasiswa.png"></center>
    <div id="rat">Rating</div> <!-- top rating  -->
    <div id="namamhs"><b>Nama Mahasiswa</b></div>
    <div id="totalsol">Total solusi</div>
    <div id="totalrel">Total relawan</div>
  </div>
  </div>

</div>
<div class="bawah">
  @Copyright | LAN Team-UNEJ-Gemastik 2017
</div>

</body>
</html> 